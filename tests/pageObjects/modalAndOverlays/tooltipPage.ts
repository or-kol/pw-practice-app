...
export class TooltipPage extends BasePage{
    ...
    private readonly TOOLTIP_BUTTON_SELECTOR = `ngx-tooltip nb-card nb-card-body`;
    ...
    async tooltipTabContentValidation(tooltipExpectedContent: string, buttonName: string): Promise<void> {
        const tooltipButtonSelector = `${this.TOOLTIP_BUTTON_SELECTOR} button:has-text("${buttonName}") >> nth=0`;
    ...
    async tooltipIconValidation(icon: string): Promise<void> {
        ...
        await this.mouseAndKeyboardInteraction.hover(`${this.TOOLTIP_BUTTON_SELECTOR} [nbtooltipicon="${icon}"]`);
    ...
    async tooltipPositionValidation(position: string, classValidation: string): Promise<void> {
        const tooltipButtonSelector = `${this.TOOLTIP_BUTTON_SELECTOR} button:has-text("${position}")`;
    ...
    async tooltipColorValidation(type: string, backgroundColor: string): Promise<void> {
        const tooltipButtonSelector = `${this.TOOLTIP_BUTTON_SELECTOR} button:has-text("${type}")`;
